...
class TenantBypassMiddleware:
    """
    Middleware to bypass tenant processing for webhook endpoints and API calls.
    This allows Evolution API and other external services to POST to webhook URLs
    without requiring authentication or tenant resolution.
    """
    
    # Legacy webhook paths (for backwards compatibility)
    BYPASS_PATHS = [
        '/api/whatsapp/webhooks/',  # Legacy WhatsApp webhook endpoints
    ]
    
    def __init__(self, get_response):
        self.get_response = get_response
    
    def _is_tenant_webhook(self, path):
        """
        Check if path matches tenant webhook pattern.
        Pattern: /tenant/{tenant_id}/whatsapp/webhooks/*
        """
        if '/whatsapp/webhooks/' in path and path.startswith('/tenant/'):
            return True
        return False
        
    def __call__(self, request):
        # Check if this is a legacy webhook URL that should bypass tenant processing
        for bypass_path in self.BYPASS_PATHS:
            if request.path.startswith(bypass_path):
                request._skip_tenant_processing = True
                logger.debug(f"Bypassing tenant for legacy webhook: {request.path}")
                return self.get_response(request)
        
        # Check for tenant webhook pattern (new multi-tenant webhook format)
        if self._is_tenant_webhook(request.path):
            request._skip_tenant_processing = True
            logger.debug(f"Bypassing tenant for tenant webhook: {request.path}")
            return self.get_response(request)
...
class TenantMiddleware:
    ...
    def __call__(self, request):
        # 1. Handle special cases that bypass tenant resolution
        ...
        # 2. Try to find a tenant_id from session, then as a fallback from the user's email
        tenant_id = request.session.get('tenant_id')
        
        # Fallback to the authenticated user's email if the session doesn't have a tenant_id
        ...
        logger.debug(f"Resolved tenant_id: {tenant_id}")
        
        # 3. Handle master user
        if tenant_id == 'master':
            logger.debug("Master user detected - skipping tenant resolution")
            set_current_db('default')
            logger.debug(f"Database set to: {get_current_db()} (master user)")
            return self.get_response(request)

        # 4. Handle regular tenants
        if tenant_id:
            try:
                request.tenant = Tenant.objects.using('default').get(tenant_id=tenant_id)
                logger.debug(f"Tenant set to {request.tenant.tenant_id}")
                set_current_db(request.tenant.db_alias)
                logger.debug(f"Database set to: {get_current_db()}")
                
                # Check for cross-tenant access, which is a good security practice
                    ...
                    if path.startswith('/tenant/'):
                        ...
                        if len(parts) > 2:
                            ...
                            if url_tenant_id != request.tenant.tenant_id:
                                logger.warning(
                                    f"Tenant mismatch! User: {request.user.email} tried to access {url_tenant_id} but belongs to {request.tenant.tenant_id}"
                                )
                ...
                logger.debug(f"Tenant not found for ID: {tenant_id}")
                request.session.flush()
                set_current_db('default')
                return redirect(settings.LOGIN_URL)
        
        # 5. If no tenant could be found at all, redirect to login
